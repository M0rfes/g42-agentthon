...
from models.index_manager import (
    index_scraped_content,
    get_vector_index,
    get_property_graph_index,
)
...
def synthesis(state: ResearchState) -> ResearchState:
    ...
    with track_step(
        "synthesis", query=query, scraped_count=len(scraped_data)
    ) as metrics:
        logger.info(
            "synthesis_node_start", query=query, scraped_count=len(scraped_data)
        )
        ...
        if not scraped_data:
            ...
            return {
                **state,
                ...
                "agents_used": state.get("agents_used", []) + ["Evaluator"],
            }
