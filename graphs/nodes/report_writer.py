...
class Citation(BaseModel):
    id: str = Field(description="Unique citation identifier used in the text, e.g., [1]")
...
class InitialDraftResult(BaseModel):
    draft_content: str = Field(description="Full markdown draft of the research report with in-text citation keys like [1], [2]")
    citations: List[Citation] = Field(description="List of all sources cited in the draft report")

class CritiqueResult(BaseModel):
    factual_assertions: List[str] = Field(description="List of 3-4 specific factual claims or stats from the draft to run through the fact-checker")
    structural_critique: str = Field(description="Stylistic, flow, and coverage critique of the draft report")

class FinalReportResult(BaseModel):
    research_report: str = Field(description="Refined, publication-grade markdown research report incorporating critique and fact-checking feedback")
    citation_source_list: List[Citation] = Field(description="Finalized and ordered list of citation sources")
...
def report_writer(state: ResearchState) -> ResearchState:
    ...
    with track_step("report_writer", query=query) as metrics:
        ...
        synthesis_context = (
            ...
            f"Empirical Findings:\n" + "\n".join([f"- {f}" for f in insight_synthesis.get("empirical_findings", [])]) + "\n\n"
            f"GraphRAG Relationships:\n" + "\n".join([f"- {r}" for r in insight_synthesis.get("entities_and_relationships", [])])
        )
        ...
        try:
            ...
            metrics["output_tokens"] += count_tokens(initial_draft.draft_content + str(initial_draft.citations))
        ...
        critique_user = (
            ...
            f"Available Citations:\n" + "\n".join([f"{c.id}: {c.title} ({c.url})" for c in initial_draft.citations])
        )
        ...
        try:
            ...
            logger.info("report_writer_critique_success", assertions_to_check=len(critique.factual_assertions))
            metrics["output_tokens"] += count_tokens(critique.structural_critique + str(critique.factual_assertions))
        except Exception as e:
            ...
            try:
                ...
                if status == "REFUTED":
                    ...
                    resolve_res_str = resolve_contradiction.invoke({
                        "claim_a": claim,
                        "claim_b": f"The claim that: {explanation}"
                    })
                    ...
                    contradictions_resolved.append({
                        ...
                        "resolution_explanation": resolve_res.get("resolution_explanation", ""),
                        "resolved_citations": resolve_res.get("resolved_citations", [])
                    })
            except Exception as e:
                logger.error("report_writer_fact_check_claim_failed", claim=claim, error=str(e))
